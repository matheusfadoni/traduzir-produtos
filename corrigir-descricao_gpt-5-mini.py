...
try:
    ...
    for indice_lote, i in enumerate(range(0, len(pendentes), TAMANHO_LOTE), start=1):
        # Se tiver limite configurado (>0) e já atingiu o limite, parar
        if LIMITE_LOTES > 0 and indice_lote > LIMITE_LOTES:
            registrar_log(f"Limite de {LIMITE_LOTES} lotes atingido. Encerrando execução.")
            break

        lote = pendentes[i:i + TAMANHO_LOTE]
        registrar_log(f"Enviando lote {indice_lote}/{LIMITE_LOTES} para a IA")
        ...
        try:
            ...
            registrar_log(f"Tokens usados no lote {i//TAMANHO_LOTE + 1}: Prompt={usage.prompt_tokens}, Limite={usage.completion_tokens}")
            registrar_log(f"Lote {i//TAMANHO_LOTE + 1}/{LIMITE_LOTES} processado com sucesso pela IA")

            conteudo_bruto = resposta.choices[0].message.content
            registrar_log("Resposta bruta da IA:\n" + conteudo_bruto)

            correcoes = tratar_resposta(conteudo_bruto)
            registrar_log(f"Correções interpretadas: {correcoes}")
            ...
            time.sleep(60)  # Intervalo antes de processar o próximo lote
